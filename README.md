...
11. LaTeX 必须按使用场景显式定界：论文卡片 JSON 字符串使用 `\\(...\\)` 表示行内公式、`\\[...\\]` 表示独立公式；“精读 Prompt”实际写入 Markdown 报告文件的公式使用 `$...$` 和 `$$...$$`；所有聊天回复中的公式统一使用 `\(...\)` 和 `\[...\]`，反斜杠不可省略，禁止用普通 `(...)`、`[...]` 或美元符号充当聊天公式定界符，也不要把公式放入代码块。普通文字不要放进数学模式。
